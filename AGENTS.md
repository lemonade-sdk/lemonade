...
14. **Client disconnect non-blocking guarantee** — Unrecoverable client socket disconnects or timeouts during inference (prefill or token generation) MUST trigger non-blocking slot cancellation (e.g. `POST /slots/{slot_id}?action=cancel`) directly to the backend HTTP port without re-entering `Router` mutex scopes or triggering nuclear model reloads.
