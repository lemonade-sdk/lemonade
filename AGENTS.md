...
14. **Client disconnect non-blocking guarantee** — Unrecoverable client socket disconnects or timeouts during inference (prefill or token generation) MUST trigger non-blocking upstream HTTP transfer aborts to the backend process without re-entering `Router` mutex scopes or triggering nuclear model reloads.
