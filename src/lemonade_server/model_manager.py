    ...
    def download_models(
        ...
        for model in models:
            if model not in self.supported_models:
                ...
                if checkpoint or recipe or reasoning or mmproj:
                    ...
                    mmproj_differs = mmproj != existing_mmproj and not (not mmproj and not existing_mmproj)
                    
                    if checkpoint_differs or recipe_differs or reasoning_differs or mmproj_differs:
                        conflicts = []
                        if checkpoint_differs:
                            conflicts.append(f"checkpoint (existing: '{existing_checkpoint}', new: '{checkpoint}')")
                        if recipe_differs:
                            conflicts.append(f"recipe (existing: '{existing_recipe}', new: '{recipe}')")
                        if reasoning_differs:
                            conflicts.append(f"reasoning (existing: {existing_reasoning}, new: {reasoning})")
                        if mmproj_differs:
                            conflicts.append(f"mmproj (existing: '{existing_mmproj}', new: '{mmproj}')")
