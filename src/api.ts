...
const DEFAULT_LEMONADE_PORT = '13305';
const DEFAULT_BASE_URL = `http://localhost:${DEFAULT_LEMONADE_PORT}`;
const LS_BASE_URL = 'lemonade_base_url';
const LS_API_KEY = 'lemonade_api_key';
const DEV_FRONTEND_PORTS = new Set(['3000', '5173', '8080', '9000']);
...
function defaultBaseUrl(): string {
  ...
  try {
    ...
    // The prototype's webpack dev server serves the UI on 8080 by default
    // (often 3000/5173/9000 in ad-hoc local runs), while lemond still listens
    // on 13305. In the packaged Lemonade web app, the UI is served by lemond
    // itself, so the browser origin is the correct API origin. If a packaged
    // app really runs on one of these dev-like ports, connect() will retry the
    // current origin before surfacing an error.
    if (DEV_FRONTEND_PORTS.has(parsed.port) && parsed.port !== DEFAULT_LEMONADE_PORT) {
      parsed.port = DEFAULT_LEMONADE_PORT;
...
function normalizeForCurrentHost(raw: string): string {
  ...
  try {
    ...
    if (parsed.hostname === 'localhost' || parsed.hostname === '127.0.0.1') {
      ...
      if (host && host !== 'localhost' && host !== '127.0.0.1') parsed.hostname = host;
    }
    return parsed.toString().replace(/\/+$/, '');
  } catch {
    return normalized;
...
function normalizeUniqueBaseUrls(urls: Array<string | null | undefined>): string[] {
  const seen = new Set<string>();
  const out: string[] = [];
  for (const url of urls) {
    if (!url) continue;
    try {
      const normalized = normalizeForCurrentHost(url);
...
function defaultBaseUrlFallbacks(primary: string): string[] {
  const origin = frontendOrigin();
  let sameHostDefaultPort: string | null = null;
  try {
    if (origin) {
      const parsed = new URL(origin);
      parsed.port = DEFAULT_LEMONADE_PORT;
      sameHostDefaultPort = parsed.toString();
    }
  } catch { /* ignore */ }

  return normalizeUniqueBaseUrls([origin, sameHostDefaultPort, DEFAULT_BASE_URL])
    .filter(candidate => candidate !== normalizeForCurrentHost(primary));
}
...
class LemonadeAPI {
  ...
  get baseUrl(): string {
    const candidate = this._baseUrlOverride || safeGetLocalStorage(LS_BASE_URL) || defaultBaseUrl();
    try {
      return normalizeForCurrentHost(candidate);
    } catch {
      return normalizeForCurrentHost(defaultBaseUrl());
    }
  }

  set baseUrl(url: string) {
    const normalized = normalizeBaseUrl(url);
    this._baseUrlOverride = normalized;
    safeSetLocalStorage(LS_BASE_URL, normalized);
  }

  get apiKey(): string {
    return this._sessionApiKey || safeGetLocalStorage(LS_API_KEY) || '';
  }

  set apiKey(key: string) {
    this._sessionApiKey = key;
    if (key) safeSetLocalStorage(LS_API_KEY, key);
    else safeRemoveLocalStorage(LS_API_KEY);
  }

  setSessionApiKey(key: string): void {
    this._sessionApiKey = key;
    if (!key) safeRemoveLocalStorage(LS_API_KEY);
  }
  ...
  async connect(): Promise<boolean> {
    ...
    try {
      ...
      const hasExplicitBaseUrl = Boolean(this._baseUrlOverride || safeGetLocalStorage(LS_BASE_URL));

      if (!hasExplicitBaseUrl) {
        const primary = this.baseUrl;
        for (const candidate of defaultBaseUrlFallbacks(primary)) {
          this._baseUrlOverride = candidate;
          try {
            await this.health();
            safeSetLocalStorage(LS_BASE_URL, candidate);
            return true;
          } catch {
      ...
      this._lastConnectionError = friendlyErrorMessage(primaryError);
