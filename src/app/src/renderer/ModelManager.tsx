...
const [searchQuery, setSearchQuery] = useState('');
  ...
  const renderLoadOptionsButton = (modelName: string) => (
    ...
    >
      {renderSettingsGearIcon()}
    </button>
  );

  const renderSettingsGearIcon = () => (
  ...
  const renderModelSettingsButton = (modelName: string) => (
    <button
      className="model-action-btn load-btn"
      onClick={(e) => {
        e.stopPropagation();
  ...
  return (
    <div className="model-manager" style={{ width: `${width}px` }}>
      ...
      <ModelSettingsModal
        model={settingsModel}
        isOpen={showModelSettingsModal}
        onCancel={() => {
          setShowModelSettingsModal(false);
          setSettingsModel(null);
        }}
        onSaved={() => {
          showSuccess('Model settings saved');
        }}
      />
                  ...
                  {visibleDownloadedModels.map(({ id: modelName, info }) => {
                    ...
                    return (
                      <div key={modelName} className="loaded-model-info">
                        ...
                        <div className="active-model-actions">
                          {renderModelSettingsButton(modelName)}
                          {canDelete && renderDeleteButton(modelName, isCollection ? 'Delete Omni Model' : 'Delete model')}
                        </div>
                      </div>
                    );
                  })}
