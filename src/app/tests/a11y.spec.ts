...
test.describe('Accessibility — model view refinements (#2424)', () => {
  ...
  async function goToModelsRefinedWithHf(page: Page): Promise<void> {
    ...
    await page.route('**/api/v1/pull/variants?**', async route => {
      ...
      const incompatible = checkpoint.includes('No-Pipeline-Image');
      await route.fulfill({
        contentType: 'application/json',
        body: JSON.stringify({
          checkpoint,
          source: 'huggingface',
          recipe: incompatible ? 'sd-cpp' : 'llamacpp',
          repo_kind: 'gguf',
          suggested_name: checkpoint.split('/').pop() || checkpoint,
          suggested_labels: [],
          mmproj_files: [],
          variants: [{ name: 'Q4_K_M', primary_file: 'model.gguf', files: ['model.gguf'], sharded: false, size_bytes: 1024 }],
        }),
      });
    });
    await goToModelsRefined(page);
