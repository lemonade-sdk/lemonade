...
assert.match(sources.chat, /handleAttachmentWheel[\s\S]*?event\.deltaMode === WheelEvent\.DOM_DELTA_LINE[\s\S]*?event\.deltaMode === WheelEvent\.DOM_DELTA_PAGE/);
assert.match(sources.chat, /const horizontalDelta = event\.deltaY \* deltaScale[\s\S]*?scrollLeft \+= horizontalDelta/);
