    ...
    for (int i = 1; i < argc; ++i) {
        ...
        if (arg == "-m" || arg == "--model") {
            ...
            if (i + 1 < argc) {
                args.mode = argv[++i];
                // Validate mode
                if (args.mode != "auto" && args.mode != "npu" && args.mode != "hybrid") {
                    throw std::runtime_error("Invalid mode: " + args.mode + " (must be auto, npu, or hybrid)");
                }
            } else {
...
void CommandLineParser::printUsage(const char* program_name) {
    std::cout << "Ryzen AI LLM Server - OpenAI API compatible server for NPU/Hybrid execution\n\n";
    ...
    std::cout << "  --mode MODE               Execution mode: auto|npu|hybrid (default: auto)\n";
