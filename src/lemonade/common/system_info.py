...
class MacOSSystemInfo(SystemInfo):
    """
    Class used to access system information in macOS.
    """

    def get_cpu_device(self) -> dict:
        """
        Retrieves CPU device information using macOS system commands.
    ...
    def get_amd_igpu_device(self, include_inference_engines: bool = False) -> dict:
        """
        Retrieves AMD integrated GPU device information.
        Note: Apple Silicon Macs don't have AMD GPUs, so this returns not available.
    ...
    def get_amd_dgpu_devices(self, include_inference_engines: bool = False) -> list:
        """
        Retrieves AMD discrete GPU device information.
        Note: Apple Silicon Macs don't have AMD GPUs, so this returns empty list.
    ...
    def get_apple_gpu_device(self, include_inference_engines: bool = False) -> dict:
        """
        Retrieves Apple GPU device information using system_profiler.

        Returns:
            dict: Apple GPU device information.
        """
        try:
            ...
            return {"available": False, "error": "No Apple GPU found"}

        except Exception as e:  # pylint: disable=broad-except
            return {"available": False, "error": f"Apple GPU detection failed: {e}"}

    def get_npu_device(self) -> dict:
        """
        Retrieves NPU device information.
        Note: Apple Silicon Macs don't have separate NPUs in the traditional sense.
    ...
    def get_device_dict(self):
        """
        Retrieves device information into a dictionary.
        Override to include Apple GPU instead of AMD GPUs.
        ...
        device_dict = {
            "cpu": self.get_cpu_device(),
            "apple_gpu": self.get_apple_gpu_device(include_inference_engines=True),
            "amd_igpu": self.get_amd_igpu_device(include_inference_engines=True),
            "amd_dgpu": self.get_amd_dgpu_devices(include_inference_engines=True),
            "npu": self.get_npu_device(),
        }
    ...
    @staticmethod
    def get_basic_processor_name() -> str:
        """
        Retrieves the basic name of the processor without core/thread details.
    ...
    def get_dict(self) -> dict:
        ...
        info_dict = super().get_dict()
        info_dict["Processor"] = self.get_basic_processor_name()
        info_dict["System Model"] = self.get_system_model()
        info_dict["Physical Memory"] = self.get_physical_memory()
        info_dict["macOS Version"] = self.get_macos_version()
        return info_dict
    ...
    def _detect_inference_engines(self, device_type: str, device_name: str) -> dict:
        ...
        Args:
            device_type: Device type ("cpu", "apple_gpu", etc.)
            device_name: Device name
