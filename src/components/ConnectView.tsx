...
const ConnectView: React.FC<ConnectViewProps> = ({ status, accountSession, onLocalDataReset, onSessionChange }) => {
  ...
  const handleClearLocalData = () => {
    const target = accountSession.role === 'admin'
      ? 'all scoped user/guest data'
      : `${describeSession(accountSession)} data`;
    const ok = window.confirm(`Clear ${target} on this device? Other signed-in users are protected unless you are admin. The server URL and theme will be kept.`);
    if (!ok) return;
    ...
    setNotice(accountSession.role === 'admin'
      ? 'Admin cleared all scoped local user data. Server URL and theme were kept.'
      : 'Current profile data was cleared. Server URL and theme were kept.');
    setError(null);
  };
    ...
    <div className="connect">
      ...
      <form className="connect__form" onSubmit={e => { e.preventDefault(); handleConnect(); }}>
        <div className="form-field">
          ...
          />
          <span className="form-field__hint">Use a full http:// or https:// URL. When the UI is served by Lemonade, the current browser address is used as the fallback.</span>
        </div>
