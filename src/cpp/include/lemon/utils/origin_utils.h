...
// implementation keeps the two policies from drifting: the WebSocket path
// previously had its own, weaker check that allowed any Origin whose host
// matched the request's Host header, which is not DNS-rebinding-safe — an
// attacker-controlled name can resolve to loopback/LAN and will send a
// matching Host header for the same name.
//
// Allowed:
