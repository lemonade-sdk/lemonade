...
// Helper function to parse and execute backend install/uninstall
...
int main(int argc, char* argv[]) {
    ...
    if (status_cmd->count() > 0) {
        ...
        // Build model_data JSON from existing flags
        nlohmann::json model_data;
        model_data["model_name"] = config.model;
        model_data["recipe"] = config.recipe;

        if (!config.checkpoints.empty()) {
            model_data["checkpoints"] = nlohmann::json::object();
            for (const auto& [key, value] : config.checkpoints) {
                model_data["checkpoints"][key] = value;
            }
        }
