...
const std::unordered_set<std::string>& EnvScrubber::common_allowed_env_vars() {
    static const std::unordered_set<std::string> common_vars = {
        "PATH",
        "HOME",
        "USER",
        "LOGNAME",
...
const std::unordered_set<std::string>& EnvScrubber::default_allowlist() {
    static const std::unordered_set<std::string> allowlist = []() {
        std::unordered_set<std::string> vars = common_allowed_env_vars();
        vars.insert("SHELL");
        vars.insert("LC_MESSAGES");
        return vars;
    }();
    return allowlist;
}
...
bool EnvScrubber::is_allowlisted_key(const std::string& key) {
    ...
    const std::string upper = to_upper_ascii(key);
    if (starts_with(upper, "MOCK_")) {
        return true;
    }
    return default_allowlist().count(upper) > 0;
}
