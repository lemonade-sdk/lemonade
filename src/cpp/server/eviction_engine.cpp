...
void EvictionEngine::evaluate_servers(double current_vram_pct) {
...

    {
        ...
        // (apply_policy_state_reconcile, enforce_llm_pool_capacity) enforces
        // by waiting on !is_loading_ before this same call — here the next
        // 5s tick just retries it instead of blocking this lock_guard-held
        // lock on a wait.
