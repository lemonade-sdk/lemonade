...
std::unique_ptr<WrappedServer> Router::create_backend_server(const ModelInfo& model_info) {
    ...
    } else if (model_info.recipe == "diffusers") {
        LOG(DEBUG, "Router") << "Creating Diffusers backend" << std::endl;
        new_server = std::make_unique<backends::DiffusersServer>(log_level, model_manager_, backend_manager_);
    } else {
