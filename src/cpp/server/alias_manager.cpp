...
void AliasManager::set_cache_dir(const std::string& cache_dir) {
    ...
    aliases_.clear();
    if (!cache_dir_.empty()) {
        fs::path cache_path = utils::path_from_utf8(cache_dir_);
                        ...
                        for (auto& [k, v] : j.items()) {
                            if (v.is_string()) {
                                ...
                                if (!norm_k.empty() && !norm_v.empty() && norm_k != norm_v) {
                                    aliases_[norm_k] = norm_v;
                                }
                            }
                        }
                    }
                } catch (const std::exception& e) {
                    LOG(WARNING, "AliasManager") << "Failed to parse aliases.json: " << e.what() << std::endl;
                }
            }
        }
    }
...
std::optional<std::string> AliasManager::resolve_alias(const std::string& alias_or_name) const {
    ...
    while (it != aliases_.end() && depth < MAX_ALIAS_CHAIN_DEPTH) {
        ...
        if (visited.count(it != aliases_.end() ? it->second : "")) {
            break; // Cycle break safety
        }
    }
...
void AliasManager::load() {
    ...
    {
        std::unique_lock lock(mutex_);
        aliases_ = std::move(loaded_aliases);
    }
