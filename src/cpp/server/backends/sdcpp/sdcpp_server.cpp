        ...
        if (resolved_backend == "rocm-stable") {
            ...
            if (!rocm_arch.empty()) {
                ...
                if (!therock_bin.empty()) {
                    ...
                    // Copy rocm_kpack.dll from TheRock to sd-server.exe directory to override System32 version.
                    fs::path therock_kpack_dll = fs::path(therock_bin) / "rocm_kpack.dll";
                    fs::path target_kpack_dll = exe_dir / "rocm_kpack.dll";
                    if (fs::exists(therock_kpack_dll)) {
                        std::error_code ec;
                        fs::copy_file(therock_kpack_dll, target_kpack_dll, fs::copy_options::overwrite_existing, ec);
                        if (!ec) {
                            LOG(INFO, "SDServer") << "Copied rocm_kpack.dll from TheRock to " << path_to_utf8(target_kpack_dll) << std::endl;
                        } else {
                            LOG(WARNING, "SDServer") << "Failed to copy rocm_kpack.dll: " << ec.message() << std::endl;
                        }
                    } else {
