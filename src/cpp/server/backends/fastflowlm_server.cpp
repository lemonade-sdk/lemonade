...
void FastFlowLMServer::install(const std::string& backend) {
    ...
    // Use the standard zip-based install flow (same as llama.cpp, whisper.cpp, etc.)
    backend_manager_->install_backend(SPEC.recipe, "npu");

    // Verify flm is now available
    std::string flm_path = get_flm_path();
    if (flm_path.empty()) {
        throw std::runtime_error("FLM installation failed - flm.exe not found");
    }
...
void FastFlowLMServer::load(const std::string& model_name,
    ...
    // Install FLM backend if needed (standard zip-based flow)
    backend_manager_->install_backend(SPEC.recipe, "npu");

    // Check FLM is ready via system-info (driver validation etc.)
    auto status = SystemInfoCache::get_flm_status();
    if (!status.is_ready()) {
...
std::string FastFlowLMServer::get_flm_path() {
    // Try the standard install directory first, then fall back to PATH search
    try {
        std::string path = BackendUtils::get_backend_binary_path(SPEC, "npu");
        LOG(INFO, "FastFlowLM") << "Found flm at: " << path << std::endl;
        return path;
    } catch (...) {
        // Fall back to PATH search (supports legacy system-wide installs)
        std::string flm_path = utils::find_flm_executable();
        if (!flm_path.empty()) {
            LOG(INFO, "FastFlowLM") << "Found flm in PATH at: " << flm_path << std::endl;
        } else {
            LOG(ERROR, "FastFlowLM") << "flm not found" << std::endl;
        }
        return flm_path;
    }
}
