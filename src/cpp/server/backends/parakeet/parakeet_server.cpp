...
void ParakeetServer::load(const std::string& model_name,
    ...
    backend_manager_->install_backend(parakeet::spec()->recipe, parakeetcpp_backend);
    ...
    std::string exe_path = BackendUtils::get_backend_binary_path(*parakeet::spec(), parakeetcpp_backend);
...
json ParakeetServer::chat_completion(const json& request) {
    return json{{"error", {
        ...
        {"type", "unsupported_operation"}, {"code", "model_not_applicable"}
    }}};
}
...
json ParakeetServer::forward_multipart_audio_data(const std::string& audio_data,
    ...
    temp_field.data = params.contains("temperature")
        ? std::to_string(params["temperature"].get<double>()) : "0.0";
...
std::unique_ptr<WrappedServer> create(const BackendContext& ctx) {
    return std::make_unique<ParakeetServer>(ctx.log_level, ctx.model_manager, ctx.backend_manager);
}

const BackendSpec* spec() { return make_spec<ParakeetServer>(descriptor); }
