    ...
    if isinstance(audio, str):
        ...
        if is_torchcodec_available() and version.parse("0.3.0") <= TORCHCODEC_VERSION:
            ...
            audio = load_audio_librosa(
                audio, sampling_rate=sampling_rate, timeout=timeout
            )
...
def load_audio_torchcodec(audio: str | np.ndarray, sampling_rate=16000) -> np.ndarray:
    ...
    audio = (
        decoder.get_all_samples().data[0].numpy()
    )  # NOTE: feature extractors don't accept torch tensors
    return audio
...
def load_audio_librosa(
    ...
    if audio.startswith("http://") or audio.startswith("https://"):
        audio = librosa.load(
            BytesIO(httpx.get(audio, follow_redirects=True, timeout=timeout).content),
            sr=sampling_rate,
        )[0]
...
def load_audio_as(
    ...
    if return_format not in ["base64", "dict", "buffer"]:
        raise ValueError(
            f"Invalid return_format: {return_format}. Must be 'base64', 'dict', or 'buffer'"
        )
            ...
            with sf.SoundFile(audio_file) as f:
                ...
                if sampling_rate is not None and sampling_rate != original_sr:
                    ...
                    audio_array = soxr.resample(
                        audio_array, original_sr, sampling_rate, quality="HQ"
                    )
                else:
...
def hertz_to_octave(
    freq: float | np.ndarray, tuning: float = 0.0, bins_per_octave: int = 12
):
    """
