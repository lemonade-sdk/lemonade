...
bool is_supported_rocm_arch(const std::string& arch) {
    // Official ROCm release zips through b1046 were gfx1151-only. Engine
    // fix/rocm-hip-arch-gfx1150 fatbins gfx1150+gfx1151 (issue #60). Accept both
    // so gfx1150 APUs do not fall through to the broken CPU auto-path; install
    // still fails clearly if an older single-arch zip lacks the needed ISA.
...
bool is_qwen35_model(const std::string& model_ref) {
    const std::string model = lowercase_copy(model_ref);
    // Also match Qwen3.6 registry models that hit the same ROCm large-ctx startup issue.
    return model.find("qwen3.5") != std::string::npos ||
           model.find("qwen3_5") != std::string::npos ||
           model.find("qwen3.6") != std::string::npos ||
           model.find("qwen3_6") != std::string::npos;
}
...
int clamp_qwen35_rocm_ctx_size(int ctx_size,
    ...
    LOG(WARNING, kLog) << "Clamping lemon-mlx ROCm context for Qwen3.5/3.6 from "
...
std::vector<json> transform_chat_stream_chunk(const json& chunk,
    ...
    for (const auto& choice : chunk["choices"]) {
        ...
        for (const auto& piece : pieces) {
            ...
            // Avoid duplicating terminal usage/timings on every split content piece.
...
void MlxServer::load(const std::string& model_name,
    ...
    if (!custom_args.empty()) {
        // Validate against the same tokenizer used for argv construction so
        // escape-based spellings cannot bypass reserved-flag checks.
        ...
        for (const auto& arg : parsed_args) {
            ...
            if (!flag.empty() && flag[0] == '-' &&
                reserved_flags.find(flag) != reserved_flags.end()) {
                throw std::runtime_error(
                    "Argument '" + flag +
    ...
    if (is_mlx_rocm_backend(backend)) {
        ...
        // Isolate CPU binary from ambient ROCm/HIP libraries (see lemonade#2740).
        // Inherited LD_LIBRARY_PATH with system ROCm causes:
        //   [hip_kernel] No ROCm back-end
        std::vector<fs::path> lib_paths;
        append_path(lib_paths, executable_dir);
        env_vars.push_back({"LD_LIBRARY_PATH", join_paths(lib_paths, /*existing=*/nullptr)});
        ...
        LOG(DEBUG, kLog) << "Configured isolated CPU runtime environment for lemon-mlx"
                         << std::endl;
    }
#elif defined(__APPLE__)
    std::vector<fs::path> lib_paths;
...
void MlxServer::forward_streaming_request(const std::string& endpoint,
    ...
    try {
        ...
        if (result.status_code != 200 && !locally_finished && !client_aborted) {
            ...
            stream_error_message =
                "backend returned HTTP " + std::to_string(result.status_code);
    ...
    if (!stream_error) {
        ...
        if (stream_error_message.empty()) {
            stream_error_message = "Streaming request failed";
        }
        json err = {
            {"error", {
        ...
        const std::string err_event = "data: " + err.dump() + "\n\n";
        if (!sink.write(err_event.c_str(), err_event.size())) {
            client_aborted = true;
        } else {
            const char* done_marker = "data: [DONE]\n\n";
        ...
        sink.done();
        LOG(ERROR, "Server") << "Streaming failed: " << stream_error_message << std::endl;
    }
}
