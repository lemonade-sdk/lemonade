...
std::string rewrite_llamacpp_help_or_type_line(const std::string& line,
    ...
    const size_t prefix_len = is_help ? 7 : 7;
...
void Server::handle_metrics(const httplib::Request& req, httplib::Response& res) {
    ...
    try {
        ...
        metrics.describe("lemonade_model_decode_token_time_count", "Number of latest per-token decode intervals observed for a loaded model.", "gauge");
        metrics.describe("lemonade_model_decode_token_time_avg_seconds", "Average latest per-token decode interval observed for a loaded model.", "gauge");
        metrics.describe("lemonade_model_decode_token_time_max_seconds", "Maximum latest per-token decode interval observed for a loaded model.", "gauge");
        ...
        for (const auto& model : loaded_models) {
            ...
            if (telemetry.contains("decode_token_times") && telemetry["decode_token_times"].is_array() &&
                !telemetry["decode_token_times"].empty()) {
                double sum = 0.0;
                double max_value = 0.0;
                size_t count = 0;
                for (const auto& dt : telemetry["decode_token_times"]) {
                    double decode_time = 0.0;
                    if (!json_number_as_double(dt, decode_time)) {
                        continue;
                    }
                    sum += decode_time;
                    max_value = std::max(max_value, decode_time);
                    count++;
                }
                if (count > 0) {
                    metrics.sample("lemonade_model_decode_token_time_count", labels, static_cast<double>(count));
                    metrics.sample("lemonade_model_decode_token_time_avg_seconds", labels, sum / count);
                    metrics.sample("lemonade_model_decode_token_time_max_seconds", labels, max_value);
                }
            }
            ...
            if (model.value("recipe", "") == "llamacpp") {
                ...
                if (parse_backend_port(model.value("backend_url", ""), backend_port)) {
                    try {
                        ...
                        if (auto backend_res = backend_client.Get("/metrics")) {
                            if (backend_res->status == 200) {
                                ...
                                while (std::getline(backend_metrics, line)) {
                                    ...
                                    std::string rewritten = rewrite_llamacpp_help_or_type_line(
                                        line, described_backend_metrics);
                                    if (rewritten.empty()) {
                        ...
                        LOG(DEBUG, "Server") << "Skipping llama.cpp metrics scrape: " << e.what() << std::endl;
                    } catch (...) {
                        LOG(DEBUG, "Server") << "Skipping llama.cpp metrics scrape: unknown error" << std::endl;
                    }
                }
            }
        }
        ...
        if (cpu_percent >= 0 && std::isfinite(cpu_percent)) {
            metrics.sample("lemonade_cpu_usage_percent", {}, cpu_percent);
        }
        ...
        if (GlobalMemoryStatusEx(&memInfo)) {
            ...
            metrics.sample("lemonade_memory_used_gb", {}, std::round(used_gb * 10.0) / 10.0);
        }
        ...
        if (sysctlbyname("hw.memsize", &physical_memory, &length, nullptr, 0) == 0) {
            ...
            metrics.sample("lemonade_memory_used_gb", {}, std::round(total_gb * 10.0) / 10.0);
        }
#endif
        ...
        if (gpu_percent >= 0 && std::isfinite(gpu_percent)) {
            metrics.sample("lemonade_gpu_usage_percent", {}, gpu_percent);
        }

        metrics.describe("lemonade_vram_used_gb", "GPU memory usage in GiB.", "gauge");
        double vram_gb = get_vram_usage();
        if (vram_gb >= 0 && std::isfinite(vram_gb)) {
            metrics.sample("lemonade_vram_used_gb", {}, vram_gb);
        }

        metrics.describe("lemonade_npu_usage_percent", "NPU utilization percentage.", "gauge");
        double npu_percent = get_npu_utilization();
        if (npu_percent >= 0 && std::isfinite(npu_percent)) {
            metrics.sample("lemonade_npu_usage_percent", {}, npu_percent);
        }
