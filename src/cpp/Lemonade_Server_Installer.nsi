; Lemonade Server Installer Script (C++ Version)
; NOTE: This file is now a template configured by CMake
; The version is automatically populated from CMakeLists.txt
...
; Define constants
!define PRODUCT_NAME "Lemonade Server"
...
Section "Install Lemonade Server" SEC01
SectionIn RO ; Read only, always installed
  DetailPrint "Installing Lemonade Server..."
  ...
  ${If} ${FileExists} "$INSTDIR\bin\lemonade-server.exe"
    DetailPrint "Stopping server using lemonade-server.exe stop..."
    nsExec::ExecToLog '"$INSTDIR\bin\lemonade-server.exe" stop'
  ...
  IfFileExists "$INSTDIR.tmp\*.*" 0 folder_in_use
    ...
    ${IfNot} ${Silent}
      MessageBox MB_YESNO "An existing Lemonade Server installation was found at $INSTDIR.$\n$\nWould you like to remove it and continue with the installation?" IDYES remove_dir
  ...
  remove_dir:
    ...
    IfFileExists "$INSTDIR\*.*" 0 continue_install
      ${IfNot} ${Silent}
        MessageBox MB_OK "Unable to remove existing installation. Please close any applications using Lemonade Server and try again."
      ${EndIf}
      Quit
  ...
  continue_install:
    ...
    CreateDirectory "$SMPROGRAMS\Lemonade Server"
    CreateShortcut "$SMPROGRAMS\Lemonade Server\Lemonade Server.lnk" "$INSTDIR\bin\lemonade-tray.exe" "" "$INSTDIR\bin\resources\static\favicon.ico" 0
    CreateShortcut "$SMPROGRAMS\Lemonade Server\Uninstall.lnk" "$INSTDIR\Uninstall.exe"
...
Section "-Add Desktop Shortcut" ShortcutSec  
  ${If} $NO_DESKTOP_SHORTCUT != "true"
    CreateShortcut "$DESKTOP\Lemonade Server.lnk" "$INSTDIR\bin\lemonade-tray.exe" "" "$INSTDIR\bin\resources\static\favicon.ico" 0
  ${EndIf}
SectionEnd
...
Function AddToStartup
  ...
  CreateShortcut "$SMSTARTUP\Lemonade Server.lnk" "$INSTDIR\bin\lemonade-tray.exe" "" "$INSTDIR\bin\resources\static\favicon.ico" 0
FunctionEnd
...
LangString MUI_TEXT_INSTALLING_TITLE "${LANG_ENGLISH}" "Installing Lemonade Server"
