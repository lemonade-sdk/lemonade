...
The fork binary is downloaded and set via LEMONADE_LLAMACPP_<BACKEND>_BIN
before bench runs — lemond picks it up at model-load time.

Usage:
  python ci/run_regression.py --forks ci/benchmark_forks.json --output ci/results
  python ci/run_regression.py --fork-filter llamacpp-rocm-nightly --model-filter Qwen3.5-4B-GGUF
  python ci/run_regression.py --fork-filter llamacpp-rocm-nightly --dry-run
"""
...
def main() -> int:
    ...
    parser.add_argument("--forks",        default="ci/benchmark_forks.json")
