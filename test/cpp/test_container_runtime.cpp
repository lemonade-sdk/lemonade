...
ContainerRunSpec sample_spec() {
    ...
    spec.labels = {{"ai.lemonade.recipe", "rocmfpx"}, {"ai.lemonade.port", "8123"}};
...
void test_run_args() {
    ...
    expect(!contains(args, "seccomp=unconfined"), "engine default seccomp profile applies");
...
void test_group_resolution() {
    ...
    // outright, and an image that defines it with a different gid than the host
    // silently grants nothing. The gid is what the device node checks.
...
void test_path_rewriting() {
    std::vector<ContainerMount> mounts = {
        {"/home/u/.cache/huggingface/hub/blobs/aaa", "/mnt/models/m.gguf", true},
        {"/data", "/data", false},
    };

    expect(ContainerRuntime::rewrite_path(mounts, "/home/u/.cache/huggingface/hub/blobs/aaa") ==
               "/mnt/models/m.gguf",
           "a mounted blob rewrites to its /mnt/models path");
...
void test_container_names() {
    ...
    expect(std::string(ContainerRuntime::managed_label()) == "ai.lemonade",
...
void test_gpu_selection() {
    expect(ContainerRuntime::gfx_name_from_target_version(110501) == "gfx1151",
           "Strix Halo target version names gfx1151");
    expect(ContainerRuntime::gfx_name_from_target_version(120001) == "gfx1201",
           "R9700 target version names gfx1201");
    expect(ContainerRuntime::gfx_name_from_target_version(90010) == "gfx90a",
           "stepping renders as a hex digit");
    expect(ContainerRuntime::gfx_name_from_target_version(0).empty(),
           "a CPU node has no gfx name");

    // KFD lists the CPU node first; HIP counts GPUs only.
    const std::vector<int> topology = {0, 110000, 110501};
    expect(ContainerRuntime::pick_gpu_index(topology, "gfx1151") == "1",
           "GPU index skips CPU nodes");
    expect(ContainerRuntime::pick_gpu_index(topology, "gfx1100") == "0",
           "first GPU is index 0");
    expect(ContainerRuntime::pick_gpu_index(topology, "gfx1201").empty(),
           "an arch that is not present selects nothing");
    expect(ContainerRuntime::pick_gpu_index(topology, "").empty(),
           "no arch selects nothing");
}
...
void test_self_mount_parsing() {
    ...
    const auto parsed = ContainerRuntime::parse_self_mounts(mounts);
    ...
    expect(parsed[0].volume && parsed[0].host_path == "lemonade-cache" &&
           ...
           "a named volume is addressed by name, not the engine's private path");
    ...
    expect(ContainerRuntime::parse_self_mounts("not json").empty(), "garbage parses to nothing");

    // The whole-container form wraps the same array.
    const std::string whole = R"([{"Id":"abc","Mounts":)" + mounts + "}]";
    expect(ContainerRuntime::parse_self_mounts(whole).size() == 3,
...
void test_sweep_filters_by_label() {
    ...
    if (!runtime.engine()) {
        std::cout << "SKIP: no container engine on PATH, sweep round-trip not exercised"
                  << std::endl;
        return;
    }
...
void test_toolbox_invocation() {
    FakeEngine fake;
    ContainerRuntime runtime(fake.runner(), HostTransport::Toolbox);
    const auto [exe, prefix] = runtime.engine_invocation(podman_engine());
    // Whether flatpak-spawn exists depends on the machine; either the host
    // prefix is applied or the engine is called directly.
    ...
    expect((spawns && prefix.size() >= 2 && prefix[0] == "--host" && prefix[1] == "podman") ||
           ...
           "toolbox transport runs the engine on the host when flatpak-spawn exists");
}

void test_native_invocation_is_bare() {
    FakeEngine fake;
    ContainerRuntime runtime(fake.runner(), HostTransport::Native);
    const auto [exe, prefix] = runtime.engine_invocation(docker_engine());
    expect(exe == "/usr/bin/docker" && prefix.empty(), "native transport calls the engine as is");
}

void test_container_transport_readiness() {
    FakeEngine fake;
    ContainerRuntime runtime(fake.runner(), HostTransport::Container);
    if (!runtime.engine()) {
        std::cout << "SKIP: no container engine on PATH" << std::endl;
        return;
    }
    // `inspect` on this process's hostname fails (fake returns exit -1), so
    // lemond cannot see itself and the readiness check must say so.
