...
bool test_wrapped_server_non_streaming_cancel() {
    ...
    {
        lemon::WrappedServer::RequestCancelScope cancel_scope(
            lemon::utils::RequestCancelToken{nullptr, [&should_cancel]() {
...
bool test_wrapped_server_nested_scope_cancel() {
    // Regression: a RequestCancelScope that carries only a flag (Router wraps a
    // caller-supplied cancel flag that way) must not drop an outer
    // connection-liveness checker, or a mid-prefill/generation disconnect is
    // never detected for non-streaming chat.
    int failures = 0;
    auto check = [&failures](bool condition, const char* name) {
        if (condition) {
            std::printf("[PASS] %s\n", name);
    ...
    const int port = backend.bind_to_any_port("127.0.0.1");
    if (port <= 0) {
        std::printf("[FAIL] failed to bind mock backend for nested-scope test\n");
        return false;
    }
    ...
    // Outer scope carries the live connection-liveness checker; the nested scope
    // carries only a flag (here it stays false). The nested scope must preserve
    // should_cancel, or only the (never-set) flag survives and the disconnect is
    // missed.
    ...
    {
        // Outer handler scope: connection-liveness checker + request flag.
        lemon::WrappedServer::RequestCancelScope handler_scope(
            lemon::utils::RequestCancelToken{
                &req_cancel,
                [&connection_closed]() { return connection_closed.load(std::memory_order_acquire); }});

        {
            // Nested scope carrying only the flag, exactly as Router::chat_completion() does.
            lemon::WrappedServer::RequestCancelScope router_scope(
                lemon::utils::RequestCancelToken{&req_cancel});
    ...
    check(
        backend_started.load(std::memory_order_acquire),
        "WrappedServer nested-scope: mock backend received the non-streaming request");
    check(
        response.contains("error") && response["error"].is_object(),
        "WrappedServer nested-scope: forward_request returned error response on client disconnect");
    check(
        response.contains("error") &&
        response["error"].value("message", "") == "Request cancelled by client",
        "WrappedServer nested-scope: cancellation error message is 'Request cancelled by client'");
    check(
        elapsed < 2000ms,
        "WrappedServer nested-scope: non-streaming request aborted promptly without waiting for backend timeout");

    std::printf(
        "WrappedServer nested-scope cancellation completed in %lld ms\n",
        static_cast<long long>(elapsed.count()));
...
int main() {
    ...
    ok = test_wrapped_server_nested_scope_cancel() && ok;
