"""
Integration test suite for client disconnect robustness in the Lemonade server.

Covers abrupt client socket disconnects (mid-stream, pre-prefill, and non-streaming)
against the running server. These are server-health checks: they assert the server
stays responsive after a client drops a connection. They do not prove that the
abandoned upstream backend request terminates; that prompt-abort behavior is covered
deterministically by the C++ `test_request_cancellation` suite (mock backend, CI ON).
...
class TestServerCancellation(ServerTestBase):
    ...
    def test_03_non_streaming_disconnect(self):
        """Non-streaming request: server stays healthy after the client closes the socket."""
        auth_hdr = self._get_auth_header_str()
    ...
    def test_04_multi_client_queue_isolation(self):
        """Server stays responsive to a new client after an earlier client drops a request."""
        auth_hdr = self._get_auth_header_str()
        body = '{"model":"default","messages":[{"role":"user","content":"hi"}],"stream":true}'
        req_payload = (
            "POST /v1/chat/completions HTTP/1.1\r\n"
