...
class EndpointTests(ServerTestBase):
    ...
    def test_012m_load_response_echoes_recipe_options(self):
        ...
        # Reset afterward (an empty save_options clears the set) so this does not bleed
        # into later tests that share the model.
        self.addCleanup(
        ...
        # A save replaces the stored options, so ctx_size is cleared here.
        resp2 = requests.post(
            f"{self.base_url}/load",
            json={
                "model_name": ENDPOINT_TEST_MODEL,
        ...
        # Relies on to_json() emitting only explicitly-set keys; revisit if it ever
        # starts baking in defaults.
        self.assertNotIn(
            "ctx_size",
            echoed,
