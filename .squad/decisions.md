...
# 2026-07-29: Remove GUI3 account storage partitioning

**Author:** Mattingly (UI/Frontend)
**Date:** 2026-07-29T15:54:18.227Z
**Status:** Approved and committed (hash f3c93cc25ba64d7cf6bb2b5418405a7412854b40)
**Milestone:** GUI3
**Branch:** kpoin/ui-testing
**Related decision:** 2026-06-05 (scoped-state migration)

## Decision

Remove GUI3 account, user, guest, and shared-space concepts from the React client entirely. Client state persists as browser-local, unscoped keys under `lemonade:<key>` with a one-time migration from legacy account-scoped keys.

### Storage Model

**Before:**
- Global scope: `lemonade:<key>` (all users, all accounts)
- Account scope: `lemonade:<account>:<key>` (account-specific storage)
- Guest/shared-space: `lemonade:guest:shared:<key>` (guest/shared-space storage)

**After:**
- Unscoped browser-local: `lemonade:<key>` (always stored/retrieved for this browser, no account partitioning)
- No `accountSession` API
- No login/logout/account-selection UI

### Migration Strategy

**Conflict-safe merging:**
- Conversation IDs remapped when two profiles contain different records with the same ID (append uniqueness suffix)
- Compatible arrays and objects (e.g., `modelHistory`, `chatSettings`) merged with arrays deduplicated and objects merged recursively
- Irreconcilable scalar values (e.g., two different `themeMode` strings) retained in a migration-conflict archive file before scoped keys deleted
- Migration runs once on first load after code change; archived conflicts remain in localStorage under `lemonade:_migration_conflicts` for inspection/recovery

**ModelManager refresh:**
- Added `useEffect` hooks to refresh/remount ModelManager UI when storage is cleared or scope changes
- Prevents stale state where UI shows models from a different account profile
- Calls existing `handleRefresh()` and forces component remount if necessary

### Scope

- **Shared renderer only** (`src/app/src/renderer/`)
- **No lemond backend changes** (per POC invariant #11 — many-clients-one-server)
- **No server-side changes**
- **Lemond config.json unaffected** (server configuration stays server-side)

### Wording Changes

- Removed references to "user-scoped" or "account-scoped" keys from UI labels and comments
- RouterEditorPanel terminology updated to reflect browser-local storage model
- Comments and variable names clarified for the post-account world

### Files Changed

- `src/app/src/renderer/` (React client only)
- Account selection UI components removed
- Storage helper functions updated to unscoped model
- ModelManager state management hooks added

### Approval Workflow

1. **Mattingly (initial):** Removed account UI and storage partitioning; flagged for review
2. **Lovell (review 1):** REJECTED for three gaps: conflict-safe migration, ModelManager stale state, wording
3. **Kranz (revision):** Independently implemented conflict-safe migration, ModelManager refresh, wording cleanup; validation passed
4. **Lovell (re-review):** APPROVED; confirmed scope, migration safety, refresh behavior, no server changes
5. **Kranz (release):** Committed with required trailers; all pre-commit checks passed

### Why This Matters

- **Simplifies client state management:** One storage model instead of multiple scopes
- **Preserves user data:** Migration is lossless except for irreconcilable collisions (archived for recovery)
- **Maintains invariants:** Honors lemond off-limits (invariant #6) and many-clients-one-server (invariant #11)
- **Prepares for OmniRouter:** Removes GUI3-specific account concepts; future multi-client/multi-server work will use explicit URL routing, not account session storage
- **Reduces code surface:** No account session management, no login UI, fewer state machines

### Post-Decision Notes

The legacy scoped-state migration patterns (from 2026-06-05) are now obsolete. Future work should assume all user preferences are browser-local unscoped keys. If role-based or profile-based preferences are needed in a future phase, they will be implemented as explicit URL state or server-side config, not client-side localStorage partitioning.

---

## 2026-07-30T09:11:53.829-06:00 — Logs and Telemetry UI Alignment

**Author:** Mattingly (UI/Frontend)
**Status:** Decision
**Branch:** kpoin/gui3-model-configuration

## Decision

Keep `LogEntry.line` untouched and derive a display-only message by removing the structured timestamp, severity, and source prefix. Place the Logs search control in the filter rail and reuse the telemetry search-input styling so equivalent filters share the same interaction pattern.
...
Keep MCP settings metadata in shared renderer data and present both built-in and external tools as semantic list rows. The chat add menu uses one shared tools submenu for Lemonade and external MCP entries, hides the top-level choices while open, and returns focus through an explicit back control.
...
## 2026-07-29T09:58:17.767-06:00 — User Directive: ChatView Change Reconciliation

**Author:** Kyle Poineal (via Copilot CLI)
**Status:** Directive
**Type:** Process constraint

## Directive

Avoid editing ChatView from this point. Report any ChatView changes already made so they can be reconciled. Continue account removal in other files and do not commit until overlap status is confirmed.

## Rationale

User request — captured for team memory to coordinate concurrent frontend changes.

---

## 2026-07-29T09:44:17.182-06:00 — Conflict-Safe Client Storage Migration

**Author:** Kranz (Release)
...
Scoped GUI3 storage is migrated into the unscoped browser-local keys. Conversation IDs are remapped when two profiles contain different records with the same ID, compatible arrays and object fields are merged, and irreconcilable scalar values are retained in a migration-conflict archive before scoped keys are removed.

## Implementation Details

- Conversation IDs remapped on collision (append uniqueness suffix)
- Compatible arrays/objects merged with arrays deduplicated
- Irreconcilable scalar values archived in `lemonade:_migration_conflicts` for recovery
- Migration runs once on first load; archived conflicts remain in localStorage

---

## 2026-07-27 — UI Cleanup Decisions — GUI3 Model Configuration
...
### 1. Presets Navigation Permanently Removed from GUI3

The top-level Presets view (nav button + `PresetManager` view slot) has been removed from GUI3's primary navigation.

**Rationale:** Kyle explicitly requested removal. The underlying preset infrastructure remains wired internally to the Configuration tab — only the navigation surface is removed.

### 2. Configuration Tab Layout Contract

`.detail-configuration` is the canonical class for Models detail Configuration tab content:
- `flex: 1; overflow-y: auto` — fills tab panel and scrolls
- `padding: var(--space-4) var(--space-5)` — standard detail-panel inset
- `display: flex; flex-direction: column; gap: var(--space-4)` — stacked sections

Field grids constrained to `minmax(190px, 280px)` columns (vs. default `minmax(190px, 1fr)`) to prevent unbounded stretching.

### 3. Backend Args Fields — CLI-Style, Not Structured Options

`llamacpp_args` and equivalent `*_args` fields accept raw CLI-style flags (`--gpu-layers 35 --threads 8`), NOT structured JSON options.

### 4. Preset-Dependent Tests Skipped, Not Deleted

Tests requiring the Presets view are marked `test.skip` / `test.describe.skip`, keeping them as reference for future work.

---

## 2026-07-24 — Reasoning Persistence Across Multi-Round Tool Calls

**Author:** Mattingly (Hooks/Backend integration)
**Status:** Proposal
**Area:** useChatStreaming, multi-round tool use

## Problem

When a reasoning-capable model emits `reasoning_content` tokens **before** deciding to call a tool, those tokens appear in the live stream's thinking panel but are discarded once the response finalizes. The persisted `message.thinking` field ends up empty or only contains reasoning from the *last* (non-tool) completion round.

**Root cause:** `chatCompletion` abandons local reasoning when `[DONE]` arrives with pending tool calls (onToolCalls fires instead of onDone).

## Decision

Fix entirely in `src/app/src/hooks/useChatStreaming.ts` — no backend changes. Add two closure-scoped variables in `send`:
- `accumulatedReasoning` — reasoning text from all completed tool-call rounds
- `currentRoundReasoning` — reasoning seen in the active `chatCompletion` call

**Change points:**
1. `onReasoning` callback: Set `currentRoundReasoning`; show accumulated + current in live stream
2. `onToolCalls` (before recursive `runCompletion`): Append `currentRoundReasoning` to accumulated, reset
3. `onDone`: Combine accumulated + stats.reasoning; pass combined to outer `onDone` only if it differs

## Impact

- Single-round (no tool calls): zero behavioral change
- Multi-round tool calls with reasoning: reasoning from all rounds persisted and displayed correctly
- TypeScript: compiles clean, no new types

---

## 2026-07-23T09:02 — Component Reuse Architecture
...
OmniFace inline blocks must reuse existing advanced-mode components (from ModelManager, etc.) wrapped in InlineBlock — not reimplement them from scratch. First extraction: ModelDetailPanel.

## Rationale

Kyle identified duplication. Advanced mode already has polished, feature-rich components. Rebuilding simpler versions for OmniFace doubles maintenance and gives a worse experience.

---

## 2026-07-13T10:01:01-06:00 — Unified HF Search — Remove Nav Rail Entry, Elevate Rich-Card Zone

**Author:** Mattingly (UI/Frontend)
**Status:** Implemented
**Issue:** #2564
**Branch:** kpoin/2564-hf-search-visible

## Decision

Removed the broken `hfMode` / nav-rail HF entry code path entirely. The single source of truth for HuggingFace search results is now the existing `renderHuggingFaceZone()` rich-card section, rendered inline inside `ModelListPanel`'s scroll area.

## Rationale

The nav-rail "Hugging Face" entry swapped `allModels` for `hfModelInfos` (plain objects with no download metadata) and reset filters — yielding a degraded experience. The rich-card path already had all information and actions. Deleting the broken path and routing everything through the good one is less code and better UX.

## Behavior Changes

- **Search with local matches** (e.g., "qwen"): Local list shown; "↓ N results on HuggingFace" anchor appears between list and HF zone
- **Search with no local matches** (e.g., "mistral"): HF zone renders above empty list; empty state suppressed
- **Clear search:** HF zone disappears
- **Nav rail:** No "Hugging Face" entry (removed from `ModelNavRail` and `PrimaryFilter` type)

## Files Changed

- `ModelManager.tsx`: Deleted hfSearchActive, hfModelInfos, hfMode, listModels ternary, useEffect for primaryFilter reset
- `ModelNavRail.tsx`: Removed hfCount prop, HF nav entry block, huggingface from primaryCounts
- `ModelListPanel.tsx`: Removed 'huggingface' from PrimaryFilter; added hfZoneTop, hfResultCount props
- `styles.css`: Removed .zone--hf--compact; added .hf-zone-anchor styles
- `tests/a11y.spec.ts`: Updated A152/A153 to check inline zone visibility

---

## 2026-07-13T08:31:56-06:00 — Issue #2564 — HF Results Inline in Model List Panel

**Author:** Mattingly (UI/Frontend)
**Status:** Implemented
**Issue:** #2564 — Can't search HuggingFace for models

## Decision

Activated the existing dead `renderHuggingFaceZone` code by surfacing it inline below local results in the `ModelListPanel` scroll area, gated on `hasHuggingFaceActivity` (query ≥2 chars AND loading/error/results present).

## Architecture

Added `hfZone?: React.ReactNode` to `ModelListPanelProps`. Created `.model-list-panel__scroll-area` wrapper div around the `<ul>` and `{hfZone}`, carrying the `overflow-y: auto; flex: 1; min-height: 0` that previously lived on the `<ul>`.

## CSS Adjustments

The `.row--hf` card designed for full-width detail panel (~600px) needed compact overrides at 360px list panel width. Added `.model-list-panel__scroll-area .row--hf` overrides:
- Single-column `row__summary` grid
- `row__right` flows as second row below content
- Icon downsized from 40px to 28px

## Accessibility

- `aria-label="HuggingFace search results"` on the `<section>`
- `role="status" aria-live="polite"` on loading div
- Decorative elements (`zone__dot`, `hf-zone__spinner`) marked `aria-hidden="true"`

## Tests

201 passed, 9 skipped. TypeScript: clean. Zero regressions including A152 (HF nav) and A153 (WCAG 2.1 AA axe scan).

---

## 2026-07-12T07:31:34-06:00 — Issue #2573 Option A — Unload Button in Model Picker
...
The chat page has no way to see loaded models or unload them once a conversation thread has messages (EmptyState loaded-model cards disappear). Option A adds an unload `×` button per loaded-model row directly inside the composer model picker dropdown.

## Key Decisions

1. **Option row restructured from `<button>` to `<div>` wrapper.** Each row is now `<div class="composer__model-option-row">` containing two independent buttons: select `<button role="option">` and unload `<button>`. This satisfies a11y: `role="option"` elements must not contain interactive children.

2. **Ghost × button, danger color on hover only.** Unload button invisible by default (`opacity: 0`), visible on row hover or button focus. Danger color only on × hover — preventing alarm on normal model-selection interactions.

3. **Loaded-count badge `(N)` on picker button.** Shows how many models are currently loaded at a glance. Rendered only when count > 0. Uses `var(--text-tertiary)` to stay subdued.

4. **Simple `modelPickerUnloading: string | null` state.** Mirrors existing `modelPickerLoading` pattern. While a model is unloading, its row shows `opacity: 0.6; cursor: wait` and × shows `…`. All other unload buttons disabled.

5. **`aria-live="polite"` in `.sr-only`.** Hidden live region announces "{model} unloaded" on success, consistent with existing pattern.

6. **`e.stopPropagation()` on unload click.** Prevents click from bubbling to row select.

7. **No virtual-component handling in picker unload.** `ModelManager.handleUnload` handles virtual collections. Picker unload uses `api.unloadModel(modelName)` directly — mirrors simple non-collection unload. If a collection model appears in picker, calling `api.unloadModel` on top-level model is correct (server handles cleanup).

---

## 2026-07-12T07:10:06.262-06:00 — GUI3 Loaded-Model Controls on Chat (#2573)
...
Prefer a **persistent loaded-model strip directly above the composer toolbar** in `ChatView.tsx`.

## Why This Is the Best Fit

- Stays visible after the first message, unlike current `EmptyState` cards
- Sits next to existing model picker, mode badge, preset controls — model management actions stay in one place
- Works in web and desktop without touching `lemond` or packaging
- Fits current design language better than global titlebar control or competing side panel

## Suggested Interaction

- One chip/card per loaded model with capability badge, active state, device/backend summary
- Primary action: select/switch to that model
- Secondary action: dedicated icon button to unload that model
- For virtual collections, reuse existing `ModelManager` unload semantics

## Accessibility Notes

- Keep selection and unload as separate focusable controls
- Add visible `:focus-visible` rings on both chip and unload button
