...
### 2026-07-29T09:44:17.182-06:00: GUI3 scoped-storage removal follow-up

- Scoped local storage migration now merges conversation records, unions compatible arrays, recursively merges disjoint settings, and archives irreconcilable scalar conflicts instead of silently dropping later profiles.
- Clearing client data remounts both ChatView and ModelManager so settings state is reloaded immediately; the migration marker is cleared with the rest of browser data.

### 2026-07-30T09:17:15.715Z — MCP documentation: ASSIGNED

**Revision assignment:** Kranz is NOW RESPONSIBLE for MCP API documentation update.

**Task:** Update docs/api/mcp.md with corrected MCP contract after C++ implementation fix.

**Sections to update:**
1. Backend status tool payload specification — Document exact fields returned by lemonade_list_backends after redaction (recipe, name, state, message, version; NO ction or elease_url)
2. Model info tool payload specification — Document exact fields returned by lemonade_get_model_info (all fields EXCEPT checkpoint/checkpoints; NO filesystem paths)
3. Security principle: "Read-only does not mean unfiltered; all outputs are redacted to expose only diagnostic state, never admin/filesystem/credential information."
4. Field prohibition list: Document what is explicitly excluded from each tool and why

**Reference:** Review defects outlined in 2026-07-30T09:17:15.715Z Lovell rejection note

**Timeline:** ~1 day (parallel with Aaron's C++ revision). Can start immediately with the specified defects.
