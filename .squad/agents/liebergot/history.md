...
### 2026-07-30T09:52:57.950-06:00 — MCP parity layer

* The first server-side MCP parity layer adds exactly five canonical lifecycle/diagnostic tools while retaining the original five tools and JSON-RPC behavior.
* Lifecycle tools use public ModelManager and Router APIs only: loading is limited to known, supported, downloaded, non-virtual models with bounded options; unloading requires an explicitly loaded model.
* `lemonade_list_backends` is wired through the read-only `BackendManager::get_all_backends_status()` API, and `/mcp` authentication remains exclusively in the existing server pre-route.
* The focused `lemonade-server-core` Release target compiled successfully; the full `lemond` link was blocked by an existing `build\Release\lemond.exe` file lock.
...
**Blocking defects:**
1. Backend `lemonade_list_backends` tool exposes forbidden fields (`action`, `release_url` — admin controls, backend URLs)
2. Model `lemonade_get_model_info` tool may expose filesystem paths via `checkpoint` fields
3. Test suite assertions insufficient — validate shape only, not field redaction

**Revision assignment:** **Aaron (C++)**. Liebergot LOCKED OUT of MCP C++ code.
