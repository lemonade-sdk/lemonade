...
# Ensure host model and data caches are shared through the flatpak mounts
export HF_HOME="${HF_HOME:-$HOME/.cache/huggingface}"
export LEMONADE_DATA_DIR="${LEMONADE_DATA_DIR:-$HOME/.cache/lemonade}"
...
LOCK="${XDG_RUNTIME_DIR:-/tmp}/app/${FLATPAK_ID:-ai.lemonadeserver.app}/supervisor.lock"
...
is_server_reachable() {
  if curl -sf --connect-timeout 1 -m 2 -o /dev/null "$HEALTH" 2>/dev/null; then
    return 0
  fi
  if curl -sf --connect-timeout 1 -m 2 -o /dev/null "http://127.0.0.1:$PORT/api/v1/health" 2>/dev/null; then
    return 0
  fi
  return 1
}
