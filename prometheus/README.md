...
- Label all metrics with `node=<hostname>`

Use `--instance` to override the hostname label:

```bash
python3 prometheus/lemonade-exporter.py \
  --instance my-gpu-server
```

The `node` label is used instead of `instance` to
avoid conflicting with Prometheus's auto-generated
`instance` label.
...
The exporter uses a `node` label for the hostname so
no special Prometheus configuration is required.
