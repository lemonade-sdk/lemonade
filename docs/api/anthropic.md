...
| `POST /api/messages` | Supported | Alias for clients using Lemonade's documented Anthropic route. |

The Anthropic messages route is also registered under Lemonade's standard versioned API prefixes: `/api/v0/messages`, `/api/v1/messages`, `/v0/messages`, and `/v1/messages`.
