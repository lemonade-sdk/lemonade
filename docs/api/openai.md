...
- `data` - Array of model objects with the following fields:
  ...
  - `max_context_window` - Optional integer indicating the maximum model-supported text context discovered from local static metadata. Currently populated for downloaded GGUF/llama.cpp models and installed FLM text-context models. For `collection.router` and `collection.omni` models this is an *aggregate*, not the collection's own context: `collection.router` reports the minimum `max_context_window` across the routing policy's resolved candidates (classifier components are excluded, since they never serve the final completion), and `collection.omni` reports the minimum across its chat/LLM component(s) only (image, audio, and TTS components are excluded). In both cases the value is the smallest context guaranteed to be safe across every candidate the collection could currently dispatch to, not a guarantee that every candidate supports that much context, and it can change as components are downloaded, removed, or updated. Components that are not yet downloaded (unknown context) are skipped rather than zeroing the aggregate; omitted if no component/candidate has a known context.
