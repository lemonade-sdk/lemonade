...
- **Per-million-token cost** — USD per 1M input/output tokens, from OpenRouter (per-token × 1e6) or Together (per-1M), when reported. Surfaced on `/v1/models` for display, and attached to `collection.router` decisions as illustrative `outputs.estimated_cost` (not a billing figure). A `collection.router` policy can also route automatically to the cheapest candidate using these same numbers — see [cost-based routing](../../dev/router-policy.md#cost-based-routing-routingroutertype-cost_select).
