...
| `--extra-models-dir [path]`    | Secondary directory to scan for GGUF model files. Models found here are automatically added to the model list with the `llamacpp` recipe. Supports multimodal and multi-shard models. Suggested paths: `%LOCALAPPDATA%\llama.cpp` (Windows) or `~/.cache/llama.cpp` (Linux/macOS). | None |
