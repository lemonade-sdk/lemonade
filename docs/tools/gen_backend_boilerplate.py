...
hand-maintained:
  ...
  * Marker-delimited regions of the backend reference docs, from
    ``/system-info`` ``recipes``.
  * The whole of ``src/cpp/resources/defaults.json``, composed from
    ``src/cpp/resources/defaults.globals.json`` (global keys) plus the per-recipe
    blocks of ``/internal/config/defaults`` (each descriptor's
    ``config_defaults()``).
...
GLOBALS_PATH = REPO_ROOT / "src" / "cpp" / "resources" / "defaults.globals.json"
DEFAULTS_HEADER = (
    "GENERATED by docs/tools/gen_backend_boilerplate.py -- do not hand-edit. "
    "Per-recipe sections come from each backend descriptor's config_defaults(); "
    "global keys come from resources/defaults.globals.json. Regenerate and verify "
    "with that script; CI --check fails on drift."
)
...
def compose_defaults(server_defaults_text: str) -> str:
    """Build resources/defaults.json: global keys from the globals file,
    per-recipe blocks from the server's dump."""
    globals_obj = json.loads(GLOBALS_PATH.read_text())
    globals_obj.pop("_comment", None)
    dump = json.loads(server_defaults_text)

    recipe_blocks = {}
    for key, value in dump.items():
        if key == "_generated" or key in globals_obj:
            continue
        # Descriptor config_defaults() blocks are always objects.
        if not isinstance(value, dict):
            sys.exit(
                f"'{key}' is a global config key missing from "
                f"{GLOBALS_PATH.relative_to(REPO_ROOT)}. Add it there."
            )
        recipe_blocks[key] = value

    composed = {"_generated": DEFAULTS_HEADER, **globals_obj, **recipe_blocks}
    return json.dumps(composed, indent=2, sort_keys=True) + "\n"
...
def main() -> int:
    ...
    with Lemond(binary) as server:
        ...
        defaults_text = compose_defaults(server.config_defaults_text())
    ...
    # is the canonical default config, composed by compose_defaults() from the
    # globals file and the descriptor blocks in /internal/config/defaults.
    raw_targets: dict = {
