...
def render_generation_params(recipes: dict) -> str:
    blocks = []
    for recipe in sorted(recipes):
        info = recipes[recipe]
        modes = info.get("generation_params") or {}
        if not modes:
            continue
        blocks.append(f"#### `{recipe}` — {info.get('display_name', recipe)}\n")
            ...
            for p in params:
                ...
                span = (
                    ...
                    if low is None and high is None
                        ...
                        f"{'' if low is None else json.dumps(low)}…{'' if high is None else json.dumps(high)}"
                    )
                )
                group = p.get("exclusive_group") or p.get("group") or "—"
                blocks.append(
                    "| `{n}` | {l} | {t} | {d} | {r} | {g} | {h} |".format(
                        n=p["name"],
                        l=md_escape(p.get("label", "")),
                        t=p.get("type_name", ""),
                        d=md_escape(json.dumps(p.get("default"))),
                        r=md_escape(span),
                        g=md_escape(group),
                        h=md_escape(p.get("help", "")),
                    )
                )
            blocks.append("")
    return "\n".join(blocks).rstrip()
    ...
    targets: dict = {
        TARGET_DOC: {
            "sections": {
                ...
                "backend-generation-params": render_generation_params(recipes),
            },
